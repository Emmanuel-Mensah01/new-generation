...
export default function ScienceLabSection({ lang }: ScienceLabSectionProps) {
  ...
  const subjects = [
    { icon: BookOpenIcon, label: lang === 'sw' ? 'Kiingereza' : 'English', color: 'bg-school-pink' },
    { icon: CalculatorIcon, label: lang === 'sw' ? 'Hisabati' : 'Mathematics', color: 'bg-school-yellow' },
    { icon: BeakerIcon, label: lang === 'sw' ? 'Sayansi' : 'Science', color: 'bg-school-pink' },
  ...
  return (
    <>
      ...
      <section className="py-20 bg-school-muted/40">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="animate-on-scroll-right order-2 lg:order-1">
              <div className="relative rounded-3xl overflow-hidden aspect-[4/3] shadow-card">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-school-pink-dark/30 to-transparent" />
            ...
            <div className="space-y-6 animate-on-scroll-left order-1 lg:order-2">
              ...
              <div className="bg-school-pink/5 border border-school-pink/20 rounded-2xl p-5">
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {subjects.map((subj, i) => (
              <div key={i} className="bg-school-muted/40 rounded-2xl border border-school-border p-5 text-center hover:shadow-card transition-all animate-on-scroll" style={{ transitionDelay: `${i * 60}ms` }}>
      ...
      {/* CTA */}
      <section className="py-20 hero-gradient-pink">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-6">
          <h2 className="text-3xl sm:text-4xl font-extrabold text-white">
            {lang === 'sw' ? 'Anza Safari ya Elimu ya Mtoto Wako' : "Start Your Child's Educational Journey"}
          </h2>
          <p className="text-white/80 text-lg max-w-2xl mx-auto">
            {lang === 'sw' ?'Jiunge na familia zaidi ya 400 zinazomwamini New Generation School kwa elimu bora na mazingira salama.' :'Join over 400 families who trust New Generation School for quality education and a safe, nurturing environment.'}
          </p>
