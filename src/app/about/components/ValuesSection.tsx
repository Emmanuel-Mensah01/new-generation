...
const VALUES = [
  { icon: AcademicCapIcon, en: 'Academic Excellence', sw: 'Ubora wa Kitaaluma', desc_en: 'We hold every learner to the highest academic standards.', desc_sw: 'Tunashikilia kila mwanafunzi kwa viwango vya juu zaidi vya kitaaluma.', color: 'bg-school-pink', textColor: 'text-white' },
  ...
  { icon: UserGroupIcon, en: 'Community Partnership', sw: 'Ushirikiano wa Jamii', desc_en: 'Parents, teachers and pupils work together as one family.', desc_sw: 'Wazazi, walimu na wanafunzi wanafanya kazi pamoja kama familia moja.', color: 'bg-school-pink/10 border border-school-pink/20', textColor: 'text-school-foreground' },
  { icon: GlobeAltIcon, en: 'Global Citizenship', sw: 'Uraia wa Kimataifa', desc_en: 'We prepare children to thrive in a connected, diverse world.', desc_sw: 'Tunaandaa watoto kustawi katika ulimwengu uliounganishwa na tofauti.', color: 'bg-school-pink/10 border border-school-pink/20', textColor: 'text-school-foreground' },
];
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {VALUES.map((val, i) => (
            <div key={i} className={`rounded-3xl p-7 animate-on-scroll ${val.color}`} style={{ transitionDelay: `${i * 80}ms` }}>
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 ${val.color.includes('bg-school-pink') && !val.color.includes('/') ? 'bg-white/20' : val.color.includes('yellow') ? 'bg-school-foreground/10' : 'bg-school-pink/10'}`}>
                <val.icon className={`w-6 h-6 ${val.color.includes('bg-school-pink') && !val.color.includes('/') ? 'text-white' : 'text-school-pink'}`} />
              </div>
              <h3 className={`text-lg font-extrabold mb-2 ${val.textColor}`}>{lang === 'sw' ? val.sw : val.en}</h3>
              <p className={`text-sm leading-relaxed ${val.textColor === 'text-white' ? 'text-white/80' : 'text-gray-500'}`}>
